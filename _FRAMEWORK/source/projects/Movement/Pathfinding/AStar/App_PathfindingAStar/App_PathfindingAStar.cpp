...
void App_PathfindingAStar::Update(float deltaTime)
{
	...
	bool hadInput{ HandleInput() };
	UpdateImGui();

	//UPDATE/CHECK GRID HAS CHANGED
	if (m_pGraphEditor->UpdateGraph(m_pGridGraph) || hadInput)
		CalculatePath();
}
...
void App_PathfindingAStar::Render(float deltaTime) const
{
	...
	if (m_StartPathIdx != invalid_node_index)
	{
		m_pGraphRenderer->HighlightNodes(m_pGridGraph, { m_pGridGraph->GetNode(m_StartPathIdx) }, START_NODE_COLOR);
	}

	//Render end node on top if applicable
	if (m_EndPathIdx != invalid_node_index)
	{
		m_pGraphRenderer->HighlightNodes(m_pGridGraph, { m_pGridGraph->GetNode(m_EndPathIdx) }, END_NODE_COLOR);
	}
...
void App_PathfindingAStar::CalculatePath()
{
	...
	if (m_StartPathIdx != invalid_node_index
		&& m_EndPathIdx != invalid_node_index
		&& m_StartPathIdx != m_EndPathIdx)
	{
		//BFS Pathfinding
		AStar<GridTerrainNode, GraphConnection> pathfinder{ m_pGridGraph, m_pHeuristicFunction };
		GridTerrainNode* startNode{ m_pGridGraph->GetNode(m_StartPathIdx) };
		GridTerrainNode* endNode{ m_pGridGraph->GetNode(m_EndPathIdx) };
...
void App_PathfindingAStar::CalculatePathBFS()
{
	//Check if valid start and end node exist
	if (m_StartPathIdx != invalid_node_index
		&& m_EndPathIdx != invalid_node_index
	...
	{
		//BFS Pathfinding
		BFS<GridTerrainNode, GraphConnection> pathfinder{ m_pGridGraph };
		GridTerrainNode* startNode{ m_pGridGraph->GetNode(m_StartPathIdx) };
		GridTerrainNode* endNode{ m_pGridGraph->GetNode(m_EndPathIdx) };
...
bool App_PathfindingAStar::HandleInput()
{
	bool const middleMousePressed = INPUTMANAGER->IsMouseButtonUp(InputMouseButton::eMiddle);
	if (middleMousePressed)
	{
		MouseData mouseData = { INPUTMANAGER->GetMouseData(InputType::eMouseButton, InputMouseButton::eMiddle) };
		...
		if (m_StartSelected)
			m_StartPathIdx = closestNode;
		else
			m_EndPathIdx = closestNode;
		return true;
	}
	return false;
}