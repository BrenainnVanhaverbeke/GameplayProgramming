...
void App_SteeringBehaviors::Start()
{
	...
	AddAgent(BehaviorTypes::Wander, -1);
...
void App_SteeringBehaviors::GenerateWindow()
{
	int const menuWidth{ 235 };
	unsigned int const width{ DEBUGRENDERER2D->GetActiveCamera()->GetWidth() };
	unsigned int const height{ DEBUGRENDERER2D->GetActiveCamera()->GetHeight() };
...
void App_SteeringBehaviors::DisplayControls()
{
	ImGui::Text("CONTROLS");
	ImGui::Indent();
...
void App_SteeringBehaviors::DisplayStats()
{
	ImGui::Text("STATS");
	ImGui::Indent();
...
void App_SteeringBehaviors::DisplayTitle()
{
	const char* title {"Steering Behaviours"};
	ImGui::Text(title);
	InsertSpacing(2);
}

void App_SteeringBehaviors::DisplayTrimWorld()
{
...
void App_SteeringBehaviors::DisplayButtons()
{
...
void App_SteeringBehaviors::HandleInput()
{
	if (INPUTMANAGER->IsMouseButtonUp(InputMouseButton::eLeft) && m_VisualizeTarget)
	{
		MouseData const mouseData = INPUTMANAGER->GetMouseData(InputType::eMouseButton, InputMouseButton::eLeft);
...
void App_SteeringBehaviors::UpdateUI(float deltaTime)
{
	...
	for (UINT i = 0; i < m_AgentVec.size(); ++i)
	{
		...
		if (ImGui::CollapsingHeader(headerName))
		{
				...
				}, reinterpret_cast<void*>(&m_TargetLabelsVec), m_TargetLabelsVec.size()))
			{
				...
				ImGui::SameLine();
				ImGui::PushItemWidth(100);
				ImGui::Text("%.3f", a.pAgent->GetLinearVelocity());
...
void App_SteeringBehaviors::UpdateAgents(float deltaTime)
{
	for (auto a : m_AgentVec)
	{
		if (a.pAgent)
...
void App_SteeringBehaviors::InsertSpacing(int amount) const
{
